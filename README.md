...
- **频道隔离**：提供 16 个虚拟频道，只有位于相同频道的节点可以互通语音。
- **节点发现与在线状态**：实时推送加入/离开/换频道事件，显示在线节点列表并高亮同频道成员。
- **身份管理**：为每个节点生成 UUID，可在 UI 中修改昵称，实时同步到其他节点。
- **错误提示**：当浏览器拒绝麦克风权限时给出清晰指引，方便排查。
- **真实 WebRTC 媒体流**：基于 RTCPeerConnection 建立点对点连接，实现低延迟高质量的语音转发；自动管理同频道节点的连接生命周期。
- **频道占用提示**：实时显示频道占用指示灯（绿色=空闲，红色脉冲=繁忙），尝试在已占用频道说话时播放繁忙提示音。
...
3. 通过底部 Channel Select 的左右箭头选择频道（CH-1 ~ CH-16）。
4. 按住空格或长按中央 PTT 按钮发射，松开即可停止。发射时有波形动画提示；同频道其他节点会显示来电状态。
...
| `NODE_ENV=production node --loader tsx server.ts` | 生产环境运行（需先执行 npm run build） |
